package be.maximvdw.mvdwupdater.utils.bukkit;

import be.maximvdw.mvdwupdater.ui.SendConsole;
import be.maximvdw.mvdwupdater.utils.NumberUtils;
import be.maximvdw.mvdwupdater.utils.ReflectionUtil;
import be.maximvdw.mvdwupdater.utils.ReflectionUtil.DynamicPackage;
...
public class BukkitUtils {
    ...
    public static String getServerVersion() {
        String version = "";
        try {
            String dynamicPackage = "net.minecraft.server."
                    + Bukkit.getServer().getClass().getPackage().getName().substring(23, 30);
            Class<?> serverClass = Class.forName(dynamicPackage + ".MinecraftServer");
            Object server = serverClass.getMethod("getServer").invoke(serverClass);
            version = (String) server.getClass().getMethod("getVersion").invoke(server);
        } catch (Exception ex) {
            // Unable to get the minecraft version
        }
        return version;
    }
